...
export function getConfig() {
    ...
    const EXECUTE_TIMEOUT_MS = Number.isFinite(raw) && raw >= 0 ? raw : DEFAULT_EXECUTE_TIMEOUT_MS;
    const truncateRaw = Number(process.env.GLASSBOX_TRUNCATE_BODY_CHARS ?? "");
    const TRUNCATE_BODY_CHARS = Number.isFinite(truncateRaw) && truncateRaw > 0
        ? truncateRaw
        : DEFAULT_TRUNCATE_BODY_CHARS;
    const storeRaw = Number(process.env.GLASSBOX_RUN_STORE_MAX_ENTRIES ?? "");
    const RUN_STORE_MAX_ENTRIES = Number.isFinite(storeRaw) && storeRaw > 0
        ? storeRaw
        : DEFAULT_RUN_STORE_MAX_ENTRIES;
    const levelsStr = process.env.GLASSBOX_BACKEND_LOG_LEVELS ?? DEFAULT_BACKEND_LOG_LEVELS;
