...
describe("getDomSnapshotAtTime", () => {
    it("devuelve error si run_id no existe", async () => {
        ...
        const result = await getDomSnapshotAtTime({ runStore, traceReader }, "missing", 1000);
    ...
    it("devuelve error si el run no tiene trace_path", async () => {
        ...
        runStore.put("r1", raw);
        const result = await getDomSnapshotAtTime({ runStore, traceReader: { getDomSnapshotAtTime: async () => null } }, "r1", 1000);
        expect(result.isError).toBe(true);
    ...
    it("devuelve snapshot cuando traceReader retorna texto", async () => {
        ...
        const result = await getDomSnapshotAtTime({ runStore, traceReader }, "r1", 1500);
        expect(result.isError).toBeFalsy();
        expect(result.content[0].text).toBe("<div>Hello</div>");
    });
});
//# sourceMappingURL=getDomSnapshotAtTime.test.js.map