...
export async function runDeterministicScenario(ports, input, options) {
    ...
    if (!raw) {
        return toolErrorResult("run_deterministic_scenario: run failed before storing (check runTestAndGetSummary error).");
    }
    if (input.restore_savepoint_after && input.savepoint_name?.trim()) {
        const savepoint = ports.savepoint;
        if (!savepoint) {
            return toolErrorResult("restore_savepoint_after is true but Savepoint not configured (GLASSBOX_SAVEPOINT_CONTAINER).");
        }
        try {
...
//# sourceMappingURL=runDeterministicScenario.js.map