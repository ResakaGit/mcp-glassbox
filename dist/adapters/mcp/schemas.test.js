...
import { executeWithTelemetrySchema, createSandboxSavepointSchema, runDeterministicScenarioSchema, inspectTraceCorrelationSchema, getDomSnapshotAtTimeSchema, } from "./schemas.js";
describe("executeWithTelemetrySchema", () => {
    it("acepta target_containers con nombres no vacíos", () => {
        ...
        if (result.success)
            expect(result.data.target_containers).toEqual(["backend", "db"]);
    });
...
describe("getDomSnapshotAtTimeSchema", () => {
    ...
    it("rechaza millisecond_offset negativo", () => {
        expect(getDomSnapshotAtTimeSchema.safeParse({
            run_id: "r1",
