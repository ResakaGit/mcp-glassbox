...
export const executeWithTelemetrySchema = z.object({
  ...
  entry_command: z
    ...
    .describe(
      "El comando de test a ejecutar (ej. 'npx playwright test auth.spec.ts')."
    ),
  target_containers: z
    .array(z.string().min(1, "cada container name debe ser no vacío"))
    .describe(
      "Contenedores de los que extraer logs de backend (ej. ['backend', 'db'])."
    )
    .default([]),
});

export type ExecuteWithTelemetryInput = z.infer<
  typeof executeWithTelemetrySchema
>;
...
export type RunTestAndGetSummaryInput = z.infer<
  typeof runTestAndGetSummarySchema
>;
...
export type CreateSandboxSavepointInput = z.infer<
  typeof createSandboxSavepointSchema
>;
...
export type RunDeterministicScenarioInput = z.infer<
  typeof runDeterministicScenarioSchema
>;
...
export type InspectTraceCorrelationInput = z.infer<
  typeof inspectTraceCorrelationSchema
>;
...
export type GetDomSnapshotAtTimeInput = z.infer<
  typeof getDomSnapshotAtTimeSchema
>;
