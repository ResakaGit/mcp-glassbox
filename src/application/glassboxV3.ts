...
import {
  filterNetworkForSummary,
  filterBackendLogsByLevel,
  truncateBody,
  DEFAULT_BACKEND_LOG_LEVELS,
} from "../domain/filtering.js";
...
async function resolveErrorLine(
  resolver: SourceMapResolverPort | null | undefined,
  basePath: string | undefined,
  message: string
): Promise<{ original: string; source: string; line: number; column?: number } | null> {
  if (!resolver || !basePath) return null;
...
export async function runTestAndGetSummary(
  ports: GlassboxPortsV3,
  input: RunTestAndGetSummaryInput,
  ...
  if (containerNames.length > 0) {
    try {
      containerLogs = await ports.containerLogs.getLogsSince(
        containerNames,
        startTimeMs,
        input.trace_id ? { traceId: input.trace_id } : undefined
      );
    } catch (err) {
      const msg = err instanceof Error ? err.message : String(err);
  ...
  const filteredNetwork = filterNetworkForSummary(
    browserSnapshot.network_failures,
    truncateChars
  );
  const filteredBackendLogs = filterBackendLogsByLevel(
    containerLogs,
    [...backendLevels]
  );

  const resolved_errors: Array<{ original: string; source: string; line: number; column?: number }> = [];
  ...
  for (const msg of [...browserSnapshot.console_errors, ...browserSnapshot.page_errors]) {
    ...
    if (resolved) resolved_errors.push(resolved);
  }

  const status = statusFromRunResult(
    runResult.exitCode,
    runResult.killed,
    runResult.timedOut
  );
  ...
  const summary: RunSummary = {
    ...
    resolved_errors:
      resolved_errors.length > 0 ? resolved_errors : undefined,
...
function truncateNetworkBodies(
  items: NetworkFailure[],
  maxChars: number
): NetworkFailure[] {
  return items.map((r) => ({
    ...r,
...
export async function queryTelemetry(
  ports: Pick<GlassboxPortsV3, "runStore">,
  run_id: string,
  ...
  if (query.type === "backend_logs" && query.backend_log_levels !== "full" && data && typeof data === "object" && "backend_logs" in data) {
    const logs = (data as { backend_logs: string[] }).backend_logs;
